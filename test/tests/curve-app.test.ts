...
describe.concurrent.each(appRegistryChains)(
  ...
  async (chainName) => {
    ...
    test("Curve app beacon has right USDC set", async () => {
      const USDC = await appRegistry.read.USDC();
      expect(USDC).toBe(envConfig.CURVE_STABLESWAP_USDC);
    });

    test("Curve app beacon has right owner set", async () => {
      const owner = await appRegistry.read.owner();
      expect(owner).toBe(envConfig.CURVE_STABLESWAP_OWNER);
    });

    test("Curve app beacon has right factory set", async () => { 
      const factory = await appRegistry.read.curveStableSwapFactory();
      expect(factory).toBe(envConfig.CURVE_STABLESWAP_FACTORY);
    });

    test("Curve app beacon has right pool set", async () => { 
      const pool = await appRegistry.read.curveStableSwapPool();
      expect(pool).toBe(envConfig.CURVE_STABLESWAP_POOL);
    });
  }
);
